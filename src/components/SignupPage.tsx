...
const SignupPage: React.FC<SignupPageProps> = ({ onSignup, onNavigate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const user = storage.signupUser(name, email, password);
      onSignup(user);
    } catch (err) {
    ...
    <div className="flex items-center justify-center min-h-[calc(100vh-64px)] bg-slate-50 px-4">
      ...
      >
        <div className="text-center mb-8">
            ...
            <h2 className="mt-4 text-3xl font-bold text-slate-800">Create an Account</h2>
            <p className="mt-2 text-slate-500">Start tailoring your resume in seconds.</p>
        </div>
